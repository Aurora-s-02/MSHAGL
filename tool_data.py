...
def prepocess_data_new_SA(args, dataset):
    ...
    cnn_feature = torch.from_numpy(np.stack(data_all_list['volume'], axis=0)).float()
    ...
    return t, data_feature.size()[0], data_feature, data_feature_st, graph, data_label, train_index, test_index, cnn_feature

def prepocess_data_new_MA(args, dataset):
    random.seed(args.seed)
    data_dir = args.data_dir
    label_rate = str(int(args.label_rate*100))
    seed = str(int(args.seed))
