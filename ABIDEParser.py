...
phenotype = r'D:\Python_code\GATE-CNN_GBConv\data\Phenotypic_V1_0b_preprocessed1.csv'
# phenotype = r'D:\Python_code\GATE-main\data\final.csv'
...
def get_ids(num_subjects=None):
    ...
    """
    data_path = r'D:\Python_code\GATE-CNN_GBConv\data\numbers.txt'
    # data_path = r'D:\Python_code\GATE-main\data\PD.txt'
    # subject_IDs = np.genfromtxt(os.path.join(data_folder[:-14], 'subject_IDs.txt'), dtype=str)
    # print(os.path.join(data_folder[:-14], 'subject_IDs.txt'))
    subject_IDs = np.genfromtxt(data_path, dtype=str)
...
def create_affinity_graph_from_scores(scores, subject_list):
    ...
    for l in scores:
        label_dict = get_subject_score(subject_list, l) # 获取id对应的性别和获取id对应的Fmri的出处
